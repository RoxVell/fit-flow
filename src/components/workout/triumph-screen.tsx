...
export function TriumphScreen({ records, volume, duration, onClose }: TriumphScreenProps) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed top-0 bottom-[calc(4rem+env(safe-area-inset-bottom))] left-1/2 z-50 flex w-full max-w-lg -translate-x-1/2 flex-col bg-gradient-to-b from-primary/15 via-background/95 to-background backdrop-blur-lg"
    >
      {/* Confetti layer */}
      ...
      <div className="grid min-h-0 flex-1 overflow-y-auto overscroll-y-contain">
        <div className="m-auto flex w-full max-w-sm flex-col items-center gap-6 px-6 py-10">
          {/* Trophy with glow */}
